...
void initLayer(float *layer, float *layer_copy, int layer_size, int k){
    for( k=0; k<layer_size; k++ ) layer[k] = 0.0f;
    for( k=0; k<layer_size; k++ ) layer_copy[k] = 0.0f;
}

void updateLayer(Storm *storms, float *layer, int layer_size, int k, int j, int i){
...
void copyLayer(float *layer, float *layer_copy, int layer_size, int k){
    for( k=0; k<layer_size; k++ )
        layer_copy[k] = layer[k];
}

void updateLayerWithAncillary(float *layer, float *layer_copy, int layer_size, int k){
    for( k=1; k<layer_size-1; k++ )
        layer[k] = ( layer_copy[k-1] + layer_copy[k] + layer_copy[k+1] ) / 3;
}

void locateMaximum(float *layer, float *maximum, int *positions, int layer_size, int i,int k){
    for( k=1; k<layer_size-1; k++ ) {
...
int main(int argc, char *argv[]) {
    ...
    initLayer(layer, layer_copy, layer_size, k);
    ...
    for( i=0; i<num_storms; i++) {
        ...
        /* For each particle */
        updateLayer(storms, layer, layer_size, k, j, i);

        /* 4.2. Energy relaxation between storms */
        /* 4.2.1. Copy values to the ancillary array */
        copyLayer(layer, layer_copy, layer_size, k);

        /* 4.2.2. Update layer using the ancillary values.
                  Skip updating the first and last positions */
        updateLayerWithAncillary(layer, layer_copy, layer_size, k);

        /* 4.3. Locate the maximum value in the layer, and its position */
        locateMaximum(layer, maximum, positions, layer_size, i, k);
    }
